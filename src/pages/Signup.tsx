...
const Signup = () => {
  ...
  const handleDetailsSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim() && phoneNumber.replace("+91 ", "").length === 10) {
      setStep("otp");
    }
  };
      ...
      <div className="relative z-10 w-full max-w-md p-6">
        <Card className="backdrop-blur-sm bg-card/95 border-0 shadow-2xl">
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <CardDescription className="text-muted-foreground">
                  {step === "details" ? t('createAccount') : t('enterOTP')}
                </CardDescription>
              </div>
              <LanguageSwitcher />
            </div>
          ...
          <CardContent className="space-y-6">
            {step === "details" ? <form onSubmit={handleDetailsSubmit} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="name">{t('name')}</Label>
                  <Input id="name" type="text" value={name} onChange={e => setName(e.target.value)} className="text-lg py-3" placeholder={t('enterName')} required />
                </div>
                ...
                <Button type="submit" className="w-full py-3 text-lg font-semibold" disabled={!name.trim() || phoneNumber.replace("+91 ", "").length !== 10}>{t('codeSent')}</Button>
              </form> : <form onSubmit={handleOtpSubmit} className="space-y-4">
                <Button type="button" variant="ghost" onClick={handleBackToDetails} className="p-0 h-auto font-normal text-muted-foreground hover:text-foreground">
            ...
            <div className="text-center">
              <p className="text-sm text-muted-foreground">
                {t('dontHaveAccount')} Already have an account?{" "}
